...
async function run() {
  try {
    console.log("Loading agents from DB...");
    await agents.init();
    ...
    const agent = agents.create({
      ...
      provider: "copilot",
      model: "gpt-4o-mini", // Dùng model ổn định
...
- If a message is ambiguous (e.g., "Tiền bia 300k"), ask to confirm if it's an expense to be recorded.`,
    ...
    console.log(`Successfully created agent: ${agent.name} (ID: ${agent.id})`);
    
    // Đợi 2s để đảm bảo persistAgent (async) hoàn thành
    await new Promise(resolve => setTimeout(resolve, 2000));
    console.log("Persistence likely complete.");
  } catch (err) {
