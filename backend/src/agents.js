...
async function load() {
  ...
  // SEED: Tự động thêm Agent Quản lý Quỹ Nhóm nếu chưa có
  const fundAgentName = "Quản lý Quỹ Nhóm";
  if (!agents.find(a => a.name === fundAgentName)) {
    const fundAgent = {
      id: "quan-ly-quy-nhom",
      name: fundAgentName,
...
- If a message is ambiguous (e.g., "Tiền bia 300k"), ask to confirm if it's an expense to be recorded.`,
      skills: ["Bookkeeping", "Financial Analysis"],
      active: true,
      autoUpdateContext: false
    };
    agents.push(fundAgent);
    // Cố gắng lưu lại vào DB cho lần sau (nếu key cho phép)
    persistAgent(fundAgent);
  }
